...
from graia.application import GraiaMiraiApplication, MessageChain, Friend, Group, Member, Source, FriendMessage, GroupMessage
...
class HandlerManager:
    ...
    def __init__(self, bcc: Broadcast):
        ...
        async def on_receive_friend_message(app: GraiaMiraiApplication,
            ...
            await self.__on_receive(app, friend, None, message)
        ...
        async def on_receive_group_message(app: GraiaMiraiApplication,
                                           ...
                                           member: Member,
                                           message: MessageChain):
            await self.__on_receive(app, group, member, message)
    ...
    def register(self, handler: AbstractMessageHandler, **kwargs):
        self.handlers.append({
            ...
            "ban_member": kwargs.get("ban_member", None)
        })
    ...
    async def __on_receive(self, app: GraiaMiraiApplication,
        ...
        try:
            for h in self.handlers:
                ...
                ban_member: T.Optional[T.Sequence[int]] = h["ban_member"]

                # 检查发送者是否有权限
                if isinstance(subject, Group):
                    if ((allow_group is not None) and (subject.id not in allow_group)) or (member.id in ban_member):
                        continue
